// jhcAliaSAPI.cpp : speech and loop timing interface for ALIA reasoner
//
...
int jhcAliaSAPI::SpeechRC () const 
{
  if (spin == 2)
    return web.Hearing();
...
void jhcAliaSAPI::UserVoice (const char *name)
{
...
int jhcAliaSAPI::time_params (const char *fname)
{
  jhcParam *ps = &tps;
  int ok;
...
int jhcAliaSAPI::Reset (const char *rname, const char *vname, int cvt)
{
...
int jhcAliaSAPI::add_sp_vocab (const char *fname, int rpt)
{
...
void jhcAliaSAPI::base_gram (const char *list)
{
...
void jhcAliaSAPI::sp_listen (int doit)
{
  if (spin == 1)
    sp.Listen(doit);
}
...
void jhcAliaSAPI::gram_add (const char *cat, const char *wd, int lvl)
{
  if (wd == NULL)
    return;
...
void jhcAliaSAPI::xfer_input ()
{
  const char *sent;
  int hear, m, attn = ((amode <= 0) ? 1 : Attending()), perk = 0;
...
int jhcAliaSAPI::syllables (const char *txt) const
{
  const char *t = txt;
  char t0, v = '\0';
  int sp = 1, n = 0;
...
bool jhcAliaSAPI::Accept (const char *in, int quit) 
{
...
const char *jhcAliaSAPI::NewInput (int clean)
{
  const char *last;
...
const char *jhcAliaSAPI::NewOutput () 
{
  const char *msg = NULL;
  UL32 now;
  int n;
...
const char *jhcAliaSAPI::blip_txt (int cutoff)
{
  char *end;
