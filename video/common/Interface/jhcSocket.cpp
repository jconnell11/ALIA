...
jhcSocket::~jhcSocket ()
{
  ...
  {
    shutdown(target, 2);     // SD_BOTH or SD_RDWR
	  closesocket(target);
  }
  ws_exit();
}
...
const char *jhcSocket::Address ()
{
  ...
  if (gethostname(hname, 80) != NET_ERROR)
  {
...
int jhcSocket::Listen (int myport, double timeout)
{
  ...
  if ((target == INVALID_SOCKET) || (myport != pnum))
  {
    ...
    if ((target = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET)
  	...
  	// get own ip address
    if (gethostname(myname, 80) == NET_ERROR)
      return -1;
    ...
    info.sin_port = htons((US16) myport);
    pnum = myport;
...
int jhcSocket::Connect (const char *hspec, int port, int block)
{
  ...
  // always make a new socket (possibly block until achieved, hard shutdown)
  Close();
  ...
  // try connecting to specified host (set non-blocking once established)
  if (connect(link, (LPSOCKADDR)(&info), sizeof(struct sockaddr)) == NET_ERROR)
    return 0;
  noblock(link);                         
  return 1;
}
...
int jhcSocket::Status ()
{
  ...
  // poll status of socket 
  FD_ZERO(&wr);
  FD_ZERO(&ex);
  FD_SET(link, &wr);
  FD_SET(link, &ex);
  if (select(0, NULL, &wr, &ex, &wait) == SEL_ERROR)
    return -1;
...
int jhcSocket::Close ()
{
  if (link != INVALID_SOCKET)
  {
    shutdown(link, 2);       // SD_BOTH or SD_RDWR
	  closesocket(link);
  }
  link = INVALID_SOCKET;
  return -1;
...
int jhcSocket::Any ()
{
  ...
  if (link == INVALID_SOCKET)
    return -1;
  if (recv(link, &dummy, 1, MSG_PEEK) != NET_ERROR)
    return 1;
  if (!blocked())
    return -1;
  return 0;
...
int jhcSocket::Receive (UC8 *data, int cnt, double timeout)
{
  ...
  for (i = 0; i <= n; i++)
  {
    ...
    inc = (int) recv(link, dest, (size_t)(cnt - total), 0);
    if (inc != NET_ERROR)
    {
      dest += inc;
      total += inc;
...
int jhcSocket::Transmit (const UC8 *data, int cnt, double timeout)
{
  ...
  for (i = 0; i <= n; i++)
  {
    ...
    inc = (int) send(link, src, (size_t)(cnt - total), 0);
    if (inc != NET_ERROR) 
    {
      src += inc;
      total += inc;
...
//= Send out a single byte on socket.
// will try for up to "timeout" seconds before giving up (generally immediate)
// return 1 if successful, -1 for broken connection
