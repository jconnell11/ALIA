...
public:
  ...
  jhcParam cps;
  double rdom, cdom, tween, sdev, buddy, hood;
  int cmax;
...
private:
  ...
  JCMD_DEF(vis_val_ok);
  int net2des (int& des, const jhcAliaDesc *hq) const;
  ...
  int node2trk (const jhcAliaDesc *n) const 
    {return seg->ObjTrack(rpt->VisID(n));}

  // net assertions
  jhcAliaDesc *obj_node (int t);
  ...
  jhcAliaDesc *trk2node (int t) const       
    {return rpt->NodeFor(seg->ObjID(t));}
