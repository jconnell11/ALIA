// jhcEliRWI.cpp : post-processed sensors and innate behaviors for ELI robot
//
...
void jhcEliRWI::BindBody (jhcEliBody& b)
{
  ...
  body = &b;
  base = &(b.base);          // jhcGenBase: use body->base for jhcEliBase
  neck = &(b.neck);          // jhcGenNeck: use body->neck for jhcEliNeck
  lift = &(b.lift);          // jhcGenLift: use body->lift for jhcEliLift
  arm  = &(b.arm);           // jhcGenArm:  use body->arm  for jhcEliArm
  mic  = &(b.mic);
  tk.RemoteMic(mic);         
}
...
int jhcEliRWI::vis_params (const char *fname)
{
  jhcParam *ps = &vps;
  int ok;
...
int jhcEliRWI::sacc_params (const char *fname)
{
  jhcParam *ps = &sps;
  int ok;
...
int jhcEliRWI::Defaults (const char *fname)
{
  int ok = 1;
...
int jhcEliRWI::LoadCfg (const char *fname)
{
  int ok = 1;
...
int jhcEliRWI::SaveVals (const char *fname) 
{
  int ok = 1;
...
int jhcEliRWI::SaveCfg (const char *fname) const
{
  int ok = 1;
...
void jhcEliRWI::Reset (int rob, int behaviors)
{
  jhcVideoSrc *v;
  const jhcImg *src;
...
int jhcEliRWI::Update (int voice, UL32 resume)
{
...
jtimer(6, "navigation (bg1)");
  ...
  nav.AdjustMaps((body->base).StepFwd(), (body->base).StepLeft(), (body->base).StepTurn());
...
int jhcEliRWI::ClosestFace (double front, int cnt) const
{
  jhcMatrix pos(4);
...
int jhcEliRWI::HeadAlong (jhcMatrix& head, double aim, double dev) const
{
  jhcMatrix pos(4);
  double off, best;
  int i, n = s3.PersonLim(), win = -1;
...
int jhcEliRWI::WatchPerson (int id, double blim, int bid)
{
  if (bid <= wlock) 
    return 0;
  wlock = bid;
...
void jhcEliRWI::OrientToward (const jhcMatrix *targ, double blim, int bid)
{
  double pan, tilt;
...
double jhcEliRWI::PersonErr (int id) const
{
  const jhcMatrix *targ;
...
int jhcEliRWI::SeekLoc (double tx, double ty, double sp, int bid)
{
  if (bid <= slock)
    return 0;
  slock = bid;
...
int jhcEliRWI::ServoPolar (double td, double ta, double off, double sp, int bid)
{
  if (bid <= vlock)
    return 0;
  vlock = bid;           
...
double jhcEliRWI::FrontDist (double td, double ta) const
{
...
int jhcEliRWI::Explore (double sp, int bid)
{
  if (bid <= xlock)
    return 0;
  xlock = bid;
...
int jhcEliRWI::quick_survey (int bid)
{
  double pan, tilt, atol = 3.0;
...
int jhcEliRWI::Skeleton (jhcImg& dest, double ray) const
{
  ...
  if (!dest.Valid(1, 3))
    return Fatal("Bad images to jhcEliRWI::Skeleton");
...
int jhcEliRWI::MapArm (jhcImg& dest, double ray) const
{
  jhcMatrix pos(4), off(4);
  int jt[5] = {1, 2, 3, 6, 7};
  double px, py, mx, my;
  ...
  if (!dest.Valid(1, 3))
    return Fatal("Bad images to jhcEliRWI::MapArm");
...
const jhcImg *jhcEliRWI::ArmMask (jhcImg& dest, int clr) const
{
  jhcMatrix pos(4), off(4);
  int jt[5] = {1, 2, 3, 6, 7};
...
double jhcEliRWI::ImgJt (double& ix, double& iy, int jt) const
{
  jhcMatrix pos(4);
...
double jhcEliRWI::ImgVeer (int mx, int my, int jt1, int jt0) const
{
...
//    if (s3.PersonState(t) > 0)
  ...
  (body->arm).PosGoal(target, lift->Height());
...
void jhcEliRWI::nav_img ()
{
  ...
  nav.Dists(mark2, 1);
  nav.RobotCmd(mark2, (body->base).TurnIncGoal(), (body->base).MoveIncGoal());
  nav.RobotBody(mark2);
