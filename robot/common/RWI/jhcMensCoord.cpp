...
int jhcMensCoord::SaveVals (const char *fname) 
{
  int ok = 1;
...
int jhcMensCoord::BindVideo (jhcVideoSrc *v, int vnum)
{
  body.BindVideo(v);
  return 1;
}
...
const jhcImg *jhcMensCoord::View (int num) 
{
  if (!body.NewFrame())
    return NULL;
  return body.View();
...
int jhcMensCoord::Done (int save)
{
  int status = 1;
